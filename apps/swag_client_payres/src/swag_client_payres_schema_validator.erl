%% -*- mode: erlang -*-
-module(swag_client_payres_schema_validator).
...
-spec validate(
    ...
    DefName :: swag_client_payres:param_name(),
    MsgType :: msg_type()
) ->
    ok | {error, Error :: swag_client_payres:error_reason()}.
validate(Value, DefName, MsgType) ->
    validate(Value, DefName, MsgType, swag_client_payres_schema:get()).

validate(Value, DefName, MsgType, Schema) ->
    Options = [{validator_opts, #{msg_type => MsgType}} | options()],
    case jesse:validate_definition(swag_client_payres_utils:to_list(DefName), Schema, Value, Options) of
        {ok, _} ->
            ok;
        {error, [Error]} ->
...
validate_child_schema(Value, SchemaName, State) ->
    ...
    BadRef = swag_client_payres_utils:to_list(Ref),
...
validate_format(Value, {?FORMAT, Format}, State) when
    ...
    Format =:= <<"date">>
->
    case swag_client_payres_param_validator:validate({type, erlang:binary_to_atom(Format, utf8)}, Value) of
        error ->
...
map_error_reason({'data_invalid', _Schema, Error, Data, Path0}) ->
    ...
    swag_client_payres_utils:join(".", [Description, Path]).
...
get_error_path(Path0) ->
    Mapper = fun
        (N, Acc) when is_integer(N) ->
            ["[", swag_client_payres_utils:to_binary(N), "]" | Acc];
        (X, Acc) ->
            [$., X | Acc]
    end,
    ...
    Path3 = swag_client_payres_utils:to_binary(Path2),
...
get_error_description({missing_dependency, Dependency0}, _Value) ->
    Dependency = swag_client_payres_utils:to_binary(Dependency0),
    <<"Missing dependency: ", Dependency/binary>>;
get_error_description(missing_required_property, Value) ->
    PropertyName = swag_client_payres_utils:to_binary(Value),

...
marshal(event, {p_transfer, TransferChange}) ->
    {transfer, #deposit_revert_TransferChange{payload = ff_p_transfer_codec:marshal(event, TransferChange)}};
...
marshal(revert_params, RevertParams) ->
    #deposit_revert_RevertParams{
        ...
        wallet_id = marshal(id, maps:get(body, RevertParams)),
        source_id = marshal(id, maps:get(body, RevertParams)),
...
marshal(revert_state, Revert) ->
    ...
    Adjustments = ff_deposit_revert:adjusments(Revert),
...
unmarshal(event, {created, #deposit_revert_CreatedChange{revert = Revert}}) ->
    {created, unmarshal(revert, Revert)};
unmarshal(event, {status_changed, #deposit_revert_StatusChange{status = Status}}) ->
    {status_changed, ff_deposit_revert_status_codec:unmarshal(status, Status)};
unmarshal(event, {transfer, #deposit_revert_TransferChange{payload = TransferChange}}) ->
    {p_transfer, ff_p_transfer_codec:unmarshal(event, TransferChange)};
unmarshal(event, {limit_check, #deposit_revert_LimitCheckChange{details = Details}}) ->
...
unmarshal(revert, Revert) ->
    genlib_map:compact(#{
        ...
        status => marshal(status, Revert#deposit_revert_Revert.status),
...
revert_symmetry_test() ->
    ...
    ?assertEqual(In, marshal(revert, unmarshal(revert, Encoded))).
...
revert_params_symmetry_test() ->
    ...
    ?assertEqual(In, marshal(revert_params, unmarshal(revert_params, In))).

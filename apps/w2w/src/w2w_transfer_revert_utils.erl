...
-module(w2w_transfer_revert_utils).
...
process_reverts(Index) ->
    ...
    NextIndex = lists:foldl(fun(E, Acc) -> w2w_transfer_revert_utils:apply_event(E, Acc) end, Index, WrappedEvents),
    Action = case {RevertAction, w2w_transfer_revert_utils:is_active(NextIndex)} of
        {undefined, true} ->
            continue;
        _Other ->
            RevertAction

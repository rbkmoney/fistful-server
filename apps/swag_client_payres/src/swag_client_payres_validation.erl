%% -*- mode: erlang -*-
-module(swag_client_payres_validation).

-export([prepare_request_param/3]).
-export([validate_response/2]).

-type rule()          :: schema | {required, boolean()} | swag_client_payres_param_validator:param_rule().
-type data_type()     :: 'list' | atom().
-type response_spec() :: {data_type(), swag_client_payres:param_name()} | undefined.

-type error() :: #{
    type        := error_type(),
    description => swag_client_payres:error_reason()
}.
...
-spec prepare_request_param(
    Rules :: [rule()],
    Name  :: swag_client_payres:param_name(),
    Value :: swag_client_payres:value()
) ->
    {ok,    Value :: swag_client_payres:value()} |
    {error, Error :: error()}.
...
-spec validate_response(
    Spec :: response_spec(),
    Resp :: swag_client_payres:object() | [swag_client_payres:object()] | undefined
) ->
    ok |
    {error, Error :: error()}.
...
-spec validate_param(
    ...
    Prepared :: swag_client_payres:value() | no_return().
...
validate(Rule = schema, Name, Value, MsgType) ->
    case swag_client_payres_schema_validator:validate(Value, Name, MsgType) of
        ok ->
            ok;
        {error, Reason} ->
...
validate(Rule, Name, Value, _MsgType) ->
    case swag_client_payres_param_validator:validate(Rule, Value) of
        ok ->
            ok;
        Ok = {ok, _} ->
            Ok;
...
-spec report_validation_error(Rule :: rule(), Param :: swag_client_payres:param_name()) ->
    no_return().
...
-spec report_validation_error(
    Rule        :: rule(),
    Param       :: swag_client_payres:param_name(),
    Description :: swag_client_payres:error_reason() | undefined
) ->
    no_return().
...
-spec map_error(Rule :: rule(), Description :: swag_client_payres:error_reason() | undefined) ->
   Error :: error().

...
-type signed_event() :: #{type := transfer_type(), id := id(), event := event()}.
...
-spec make_default_events(transfer_type(), map()) ->
    {ok, [event()]} |
    {error,
        _PostingsTransferError
    }.
...
wrap_events_for_parent(_ID, _TransferType, Events, undefined) ->
    Events;
wrap_events_for_parent(ID, TransferType, Events, _Parent) ->
    [{transfer, sign_event(ID, TransferType, Ev)} || Ev <- Events].

sign_event(ID, TransferType, Event) ->
    #{id => ID, type => TransferType, event => Event}.
...
do_process_transfer(transfer, Transfer) ->
    Child = get_last_child(childs(Transfer)),
    process_transfer(Child);
do_process_transfer(_Activity, Transfer) ->
    Handler = type_to_handler(transfer_type(Transfer)),
    Handler:process_transfer(Transfer).

get_last_child(Transfer) when
    Transfer =:= undefined orelse
    Transfer =:= []
->
    undefined;
get_last_child([H | _T]) ->
...
-spec apply_event(event() | legacy_event(), ff_maybe:maybe(transfer(T))) ->
    transfer(T).
apply_event(Ev, undefined) ->
    apply_event_(maybe_migrate(Ev, undefined), undefined);
apply_event(Ev, T) ->

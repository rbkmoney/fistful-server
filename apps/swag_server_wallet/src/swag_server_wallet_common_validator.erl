%% -*- mode: erlang -*-
-module(swag_server_wallet_common_validator).
...
-type result() ::
    ok | {ok, Prepared :: swag_server_wallet:value()} | error | {error, Message :: term()}.
...
validate({type, 'integer'}, Value0) ->
    try
        Value = swag_server_wallet_utils:to_int(Value0),
        {ok, Value}
    catch
        error:badarg ->
            error
...
validate({type, 'boolean'}, Value) ->
    case swag_server_wallet_utils:to_lower(Value) of
        <<"true">> ->  {ok, true};
        <<"false">> -> {ok, false};
        _ -> error
    end;
...
validate({type, 'float'}, Value) ->
    try
        {ok, swag_server_wallet_utils:to_float(Value)}
    catch
        error:badarg ->
            error
    end;
...
-spec validate_between(Value :: swag_server_wallet:value(), Min :: integer(), Max :: integer()) ->
    ok | no_return().
...
-spec validate_base64(Value :: swag_server_wallet:value()) ->
    ok | no_return().

%% -*- mode: erlang -*-
-module(swag_client_wallet_procession).
...
-type opts()   :: swag_client_wallet:transport_opts().
-type object() :: swag_client_wallet:object().
-type url()    :: string().
-type method() :: atom().
...
-type request_spec() :: [{
    swag_client_wallet:param_name(),
    #{source := param_source(), rules := [swag_client_wallet_validation:rule()]}
}].

-type response_spec() :: swag_client_wallet_validation:response_spec().
...
-spec process_request(method(), url(), params(), request_spec(), opts()) ->
    {ok, code(), list(), body()} |
    {error, {request_validation_failed,  { swag_client_wallet_validation:error(), BadParam :: atom()}}} |
    {error, _Reason}.
...
-spec process_response(response_spec(), body()) ->
    {ok, object()} |
    {error, {response_validation_failed, swag_client_wallet_validation:error(), _Response}}.
...
prepare_url(Url, Params, Qs) ->
    swag_client_wallet_utils:fill_url(Url, Params, Qs).
...
validate_request_param({Name, #{rules := Rules, source := Source}}, Params) ->
    ...
    case swag_client_wallet_validation:prepare_request_param(Rules, Name, Value) of
        {ok, _} ->
            ok;
        {error, Reason} ->
...
get_value(qs_val, Name, Params) ->
    ...
    get_opt(swag_client_wallet_utils:to_binary(Name), QueryParams);
get_value(header, Name, Params) ->
    HeaderParams = maps:get(header, Params),
    get_opt(swag_client_wallet_utils:to_binary(Name), HeaderParams);
get_value(binding, Name, Params) ->
    BindingParams = maps:get(binding, Params),
    get_opt(swag_client_wallet_utils:to_binary(Name), BindingParams).
...
validate_response(Spec, RespBody) ->
    case swag_client_wallet_validation:validate_response(Spec, RespBody) of
        ok ->
            ok;
        {error, Error} ->

-module(swag_server_wallet_validation).

-export([prepare_request_param/5]).
-export([validate_response/4]).

-type rule()          :: schema | {required, boolean()} | swag_server_wallet_param_validator:param_rule().
-type data_type()     :: 'list' | atom().
-type response_spec() :: {data_type(), swag_server_wallet:param_name()} | undefined.

-type error() :: #{
    type        := error_type(),
    param_name  => swag_server_wallet:param_name(),
    description => swag_server_wallet:error_reason()
}.
...
-type validation_opts() :: #{
    schema => swag_server_wallet_schema_validator:schema_validation_opts(),
...
-spec prepare_request_param(
    ...
    {ok,    Value :: swag_server_wallet:value()} |
    {error, Error :: error()}.
...
-spec validate_response(
    OperationID :: swag_server_wallet:operation_id(),
    Spec :: response_spec(),
    Resp :: swag_server_wallet:object() | [swag_server_wallet:object()] | undefined,
    ValidationOpts :: validation_opts()
) ->
    ok |
    {error, Error :: error()}.
...
-spec validate_param(
    ...
    Prepared :: swag_server_wallet:value() | no_return().
...
validate(OperationID, Rule = schema, Name, Value, MsgType, ValidationOpts) ->
    case swag_server_wallet_schema_validator:validate(OperationID, Value, Name, MsgType, ValidationOpts) of
        ok ->
            ok;
        {error, Reason} ->
...
validate(OperationID, Rule, Name, Value, MsgType, ValidationOpts) ->
    case swag_server_wallet_param_validator:validate(OperationID, Rule, Name, Value, MsgType, ValidationOpts) of
        ok ->
            ok;
        Ok = {ok, _} ->
            Ok;
...
-spec report_validation_error(Rule :: rule(), Param :: swag_server_wallet:param_name()) ->
    no_return().
...
-spec report_validation_error(
    Rule        :: rule(),
    Param       :: swag_server_wallet:param_name(),
    Description :: swag_server_wallet:error_reason() | undefined
) ->
    no_return().
...
-spec map_error(Rule :: rule(), Description :: swag_server_wallet:error_reason() | undefined) ->
   Error :: error().

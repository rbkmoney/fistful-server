...
marshal(event, {status_changed, Status}) ->
    {status_changed, #dep_rev_adj_StatusChange{status = ff_deposit_revert_status_codec:marshal(status, Status)}};
marshal(event, {p_transfer, TransferChange}) ->
    {transfer, #dep_rev_adj_TransferChange{payload = ff_p_transfer_codec:marshal(event, TransferChange)}};
...
unmarshal(event, {created, #dep_rev_adj_CreatedChange{adjustment = Adjustment}}) ->
    {created, unmarshal(adjustment, Adjustment)};
unmarshal(event, {status_changed, #dep_rev_adj_StatusChange{status = Status}}) ->
    {status_changed, unmarshal(status, Status)};
unmarshal(event, {transfer, #dep_rev_adj_TransferChange{payload = TransferChange}}) ->

%% -*- mode: erlang -*-
-module(swag_server_wallet_param_validator).

-export([validate/6]).

-type param_base_rule() ::
    swag_server_wallet_common_validator:param_rule() |
    {enum, [atom()]} |
...
-type name()     :: swag_server_wallet:param_name().
...
validate_with_context(Rule, Value, Meta, ValidationOpts) ->
    case swag_server_wallet_custom_validator:validate_param(Rule, Value, Meta, ValidationOpts) of
...
do_validate({enum, Values}, Value, _Meta, _Opts) ->
    try
        FormattedValue = swag_server_wallet_utils:binary_to_existing_atom(Value, utf8),
...
do_validate({RuleName, _} = Rule, Value, _Meta, _Opts) when
    ...
    RuleName =:= format
->
    swag_server_wallet_common_validator:validate(Rule, Value).

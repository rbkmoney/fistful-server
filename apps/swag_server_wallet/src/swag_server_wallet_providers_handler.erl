...
%% basic handler
-module(swag_server_wallet_providers_handler).
...
allowed_methods(
    Req,
    State = #state{
        operation_id = 'GetProviderIdentityClass'
    }
) ->
    {[<<"GET">>], Req, State};
...
allowed_methods(
    Req,
    State = #state{
        operation_id = 'GetProviderIdentityLevel'
    }
) ->
    {[<<"GET">>], Req, State};
...
allowed_methods(
    Req,
    State = #state{
        operation_id = 'ListProviderIdentityClasses'
    }
) ->
    {[<<"GET">>], Req, State};
...
allowed_methods(
    Req,
    State = #state{
        operation_id = 'ListProviderIdentityLevels'
    }
) ->
    {[<<"GET">>], Req, State};
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'GetProvider' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'GetProviderIdentityClass' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'GetProviderIdentityLevel' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'ListProviderIdentityClasses' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'ListProviderIdentityLevels' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'ListProviders' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
get_request_spec('GetProviderIdentityClass') ->
    [
        {'X-Request-ID', #{
            source => header,
            rules  => [{type, 'binary'}, {max_length, 32}, {min_length, 1}, true
...
get_request_spec('GetProviderIdentityLevel') ->
    [
        {'X-Request-ID', #{
            source => header,
            rules  => [{type, 'binary'}, {max_length, 32}, {min_length, 1}, true
...
get_request_spec('ListProviderIdentityClasses') ->
    [
        {'X-Request-ID', #{
            source => header,
            rules  => [{type, 'binary'}, {max_length, 32}, {min_length, 1}, true
...
get_request_spec('ListProviderIdentityLevels') ->
    [
        {'X-Request-ID', #{
            source => header,
            rules  => [{type, 'binary'}, {max_length, 32}, {min_length, 1}, true
...
get_request_spec('ListProviders') ->
    [
        {'X-Request-ID', #{
            source => header,
            rules  => [{type, 'binary'}, {max_length, 32}, {min_length, 1}, true
...
get_response_spec('GetProvider', 200) ->
    {'Provider', 'Provider'};

get_response_spec('GetProvider', 400) ->
    {'BadRequest', 'BadRequest'};

get_response_spec('GetProvider', 401) ->
    undefined;

get_response_spec('GetProvider', 404) ->
    undefined;

get_response_spec('GetProviderIdentityClass', 200) ->
    {'IdentityClass', 'IdentityClass'};

get_response_spec('GetProviderIdentityClass', 400) ->
    {'BadRequest', 'BadRequest'};

get_response_spec('GetProviderIdentityClass', 401) ->
    undefined;

get_response_spec('GetProviderIdentityClass', 404) ->
    undefined;

get_response_spec('GetProviderIdentityLevel', 200) ->
    {'IdentityLevel', 'IdentityLevel'};

get_response_spec('GetProviderIdentityLevel', 400) ->
    {'BadRequest', 'BadRequest'};

get_response_spec('GetProviderIdentityLevel', 401) ->
    undefined;

get_response_spec('GetProviderIdentityLevel', 404) ->
    undefined;

get_response_spec('ListProviderIdentityClasses', 200) ->
    {'list', 'IdentityClass'};

get_response_spec('ListProviderIdentityClasses', 400) ->
    {'BadRequest', 'BadRequest'};

get_response_spec('ListProviderIdentityClasses', 401) ->
    undefined;

get_response_spec('ListProviderIdentityClasses', 404) ->
    undefined;

get_response_spec('ListProviderIdentityLevels', 200) ->
    {'list', 'IdentityLevel'};

get_response_spec('ListProviderIdentityLevels', 400) ->
    {'BadRequest', 'BadRequest'};

get_response_spec('ListProviderIdentityLevels', 401) ->
    undefined;

get_response_spec('ListProviderIdentityLevels', 404) ->
    undefined;

get_response_spec('ListProviders', 200) ->
    {'list', 'Provider'};

get_response_spec('ListProviders', 400) ->
    {'BadRequest', 'BadRequest'};

get_response_spec('ListProviders', 401) ->
    undefined;

...
%% basic handler
-module(swag_server_wallet_webhooks_handler).
...
allowed_methods(
    Req,
    State = #state{
        operation_id = 'DeleteWebhookByID'
    }
) ->
    {[<<"DELETE">>], Req, State};
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'CreateWebhook' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'DeleteWebhookByID' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'GetWebhookByID' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'GetWebhooks' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
get_request_spec('CreateWebhook') ->
    [
        {'X-Request-ID', #{
            source => header,
...
get_request_spec('DeleteWebhookByID') ->
    [
        {'X-Request-ID', #{
            source => header,
            rules  => [{type, 'binary'}, {max_length, 32}, {min_length, 1}, true
...
get_request_spec('GetWebhookByID') ->
    [
        {'X-Request-ID', #{
            source => header,
            rules  => [{type, 'binary'}, {max_length, 32}, {min_length, 1}, true
...
get_response_spec('CreateWebhook', 201) ->
    {'Webhook', 'Webhook'};
...
get_response_spec('CreateWebhook', 422) ->
    {'InvalidOperationParameters', 'InvalidOperationParameters'};

get_response_spec('DeleteWebhookByID', 204) ->
    undefined;

get_response_spec('DeleteWebhookByID', 400) ->
    {'BadRequest', 'BadRequest'};

get_response_spec('DeleteWebhookByID', 401) ->
    undefined;

get_response_spec('DeleteWebhookByID', 404) ->
    undefined;

get_response_spec('DeleteWebhookByID', 422) ->
    {'InvalidOperationParameters', 'InvalidOperationParameters'};

get_response_spec('GetWebhookByID', 200) ->
    {'Webhook', 'Webhook'};

get_response_spec('GetWebhookByID', 400) ->
    {'BadRequest', 'BadRequest'};

get_response_spec('GetWebhookByID', 401) ->
    undefined;

get_response_spec('GetWebhookByID', 404) ->
    undefined;

get_response_spec('GetWebhookByID', 422) ->
    {'InvalidOperationParameters', 'InvalidOperationParameters'};

get_response_spec('GetWebhooks', 200) ->
    {'list', 'Webhook'};

get_response_spec('GetWebhooks', 400) ->
    {'BadRequest', 'BadRequest'};

get_response_spec('GetWebhooks', 401) ->
    undefined;

get_response_spec('GetWebhooks', 422) ->
    {'InvalidOperationParameters', 'InvalidOperationParameters'};

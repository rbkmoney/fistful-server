%%%
%%% Session machine
%%%
%%% TODOs
%%%
%%%  - The way we ask `fistful` for a machinery backend smells like a circular
%%%    dependency injection.
%%%  - Dehydrate events upon saving.
%%%

-module(ff_session_machine).
-behaviour(machinery).

%% API

-export([session/1]).

-export([create/4]).
-export([get/2]).
-export([events/3]).
-export([repair/3]).

%% machinery

-export([init/4]).
-export([process_timeout/3]).
-export([process_repair/4]).
-export([process_call/4]).

%%
%% Internal types
%%

-type id() :: machinery:id().

-type machine()      :: ff_machine:machine(event()).
-type result()       :: ff_machine:result(event()).
-type handler_opts() :: machinery:handler_opts(_).
-type handler_args() :: machinery:handler_args(_).

-type st()           :: ff_machine:st(session()).
-type session()      :: ff_session:session().
-type event()        :: ff_session:event().
-type ctx()          :: ff_ctx:ctx().
-type ns()           :: machinery:namespace().

-export_type([st/0]).

%% Pipeline

-import(ff_pipeline, [do/1, unwrap/1]).

%%
%% API
%%

-spec session(st()) -> session().

session(St) ->
    ff_machine:model(St).

%%

-spec create(ns(), id(), [event()], ctx()) ->
    ok | {error, exists}.

create(NS, ID, Events, Ctx) ->
    do(fun () ->
        unwrap(machinery:start(NS, ID, {Events, Ctx}, backend(NS)))
    end).

-spec get(ns(), id()) ->
    ff_map:result(st()).
get(NS, ID) ->
    ff_machine:get(ff_session, NS, ID).

-spec events(ns(), id(), machinery:range()) ->
    {ok, [{integer(), ff_machine:timestamped_event(event())}]} |
    {error, notfound}.

events(NS, ID, Range) ->
    do(fun () ->
        #{history := History} = unwrap(machinery:get(NS, ID, Range, backend(NS))),
        [{EventID, TsEv} || {EventID, _, TsEv} <- History]
    end).

-spec repair(ns(), id(), ff_repair:scenario()) ->
    ok | {error, notfound | working}.
repair(NS, ID, Scenario) ->
    machinery:repair(NS, ID, Scenario, backend(NS)).

%% machinery callbacks

-spec init({[event()], ctx()}, machine(), handler_args(), handler_opts()) ->
    result().
init({Events, _}, #{}, _, _Opts) ->
    #{
        events => ff_machine:emit_events(Events),
        action => continue,
        aux_state => #{ctx => ff_ctx:new()}
    }.

-spec process_timeout(machine(), handler_args(), handler_opts()) ->
    result().
process_timeout(Machine, _, _Opts) ->
    State = ff_machine:collapse(ff_session, Machine),
    #{events := Events} = Result = ff_session:process_session(session(State)),
    Result#{
        events => ff_machine:emit_events(Events)
    }.

-spec process_call(any(), machine(), handler_args(), handler_opts()) ->
    {ok, result()}.
process_call(_CallArgs, #{}, _, _Opts) ->
    {ok, #{}}.

-spec process_repair(ff_repair:scenario(), machine(), handler_args(), handler_opts()) ->
    result().
process_repair(Scenario, Machine, _Args, _Opts) ->
    ScenarioProcessors = #{
        set_session_result => fun(Args, RMachine) ->
            State = ff_machine:collapse(ff_session, RMachine),
            ff_session:set_session_result(Args, session(State))
        end
    },
    ff_repair:apply_scenario(ff_session, Machine, Scenario, ScenarioProcessors).

%%
%% Internals
%%

backend(NS) ->
    fistful:backend(NS).

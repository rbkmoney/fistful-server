...
from_swag(destination_resource, Resource = #{
    ...
    {crypto_wallet, genlib_map:compact(#{
        id   => CryptoWalletID,
        data => from_swag(crypto_wallet_currency, Resource)
    })};

from_swag(crypto_wallet_currency, #{<<"currency">> := Currency} = Resource) ->
    genlib_map:compact(#{
        currency => from_swag(crypto_wallet_currency_name, Currency),
...
to_swag(destination_resource, {crypto_wallet, CryptoWallet}) ->
    ...
    }, to_swag(crypto_data, maps:get(data, CryptoWallet))));

to_swag(pan_last_digits, MaskedPan) ->
    wapi_utils:get_last_pan_digits(MaskedPan);

to_swag(crypto_data, #{currency := bitcoin})            -> #{<<"currency">> => <<"Bitcoin">>};
to_swag(crypto_data, #{currency := litecoin})           -> #{<<"currency">> => <<"Litecoin">>};
to_swag(crypto_data, #{currency := bitcoin_cash})       -> #{<<"currency">> => <<"BitcoinCash">>};
to_swag(crypto_data, #{currency := ethereum})           -> #{<<"currency">> => <<"Ethereum">>};
to_swag(crypto_data, #{currency := zcash})              -> #{<<"currency">> => <<"Zcash">>};
to_swag(crypto_data, #{currency := ripple, tag := Tag}) -> #{<<"currency">> => <<"Ripple">>, <<"tag">> => Tag};
to_swag(crypto_data, #{currency := ripple})             -> #{<<"currency">> => <<"Ripple">>};

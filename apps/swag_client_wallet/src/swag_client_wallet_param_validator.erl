%% -*- mode: erlang -*-
-module(swag_client_wallet_param_validator).
...
-spec validate(Rule :: param_rule(), Param :: swag_client_wallet:value()) ->
    ok | {ok, Prepared :: swag_client_wallet:value()} | error.
...
validate({type, 'int64'}, Value0) ->
    try
        Value = swag_client_wallet_utils:to_int(Value0),
...
validate({type, 'float'}, Value) ->
    try
        {ok, swag_client_wallet_utils:to_float(Value)}
    catch
        error:badarg ->
            error
    end;
...
validate({type, 'boolean'}, Value) ->
    V = swag_client_wallet_utils:to_lower(Value),
    try
        case swag_client_wallet_utils:binary_to_existing_atom(V, utf8) of
...
validate({enum, Values}, Value) ->
    try
        FormattedValue = swag_client_wallet_utils:binary_to_existing_atom(Value, utf8),
...
-spec validate_between(Value :: swag_client_wallet:value(), Min :: integer(), Max :: integer()) ->
    ok | no_return().
...
-spec validate_base64(Value :: swag_client_wallet:value()) ->
    ok | no_return().

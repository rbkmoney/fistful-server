%% -*- mode: erlang -*-
-module(swag_server_wallet_utils).
...
-spec get_body(Req) -> {ok, binary(), Req} | {error, swag_server_wallet:error_reason()} when Req::cowboy_req:req().
get_body(Req) ->
    get_body(Req, #{}).

-spec get_body(Req, cowboy_req:read_body_opts()) -> {ok, binary(), Req} | {error, swag_server_wallet:error_reason()} when Req::cowboy_req:req().
get_body(Req, Opts) ->
...
-spec get_operation_id(cowboy_req:req(), swag_server_wallet_router:init_opts()) -> swag_server_wallet:operation_id() | undefined.

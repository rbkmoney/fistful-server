%% -*- mode: erlang -*-
-module(swag_server_wallet_schema_validator).
...
-type msg_type() :: swag_server_wallet_validation:msg_type().
-type schema_validation_opts() :: #{msg_type() => validation_ruleset()}.
-type validation_opts() :: swag_server_wallet_validation:validation_opts().
...
-type context() ::
    #{
        ...
        msg_type     := msg_type(),
        definition_name     := swag_server_wallet:param_name(),
...
-spec validate(
    ...
    Value      :: swag_server_wallet:value(),
    DefinitionName    :: swag_server_wallet:param_name(),
    MsgType    :: msg_type(),
    ValidationOpts :: validation_opts()
) ->
    ok | {error, Error :: swag_server_wallet:error_reason()}.
validate(OperationID, Value, DefinitionName, MsgType, ValidationOpts) ->
    validate(OperationID, Value, DefinitionName, MsgType, swag_server_wallet_schema:get(), ValidationOpts).
...
validate(OperationID, Value, DefinitionName, MsgType, Schema, ValidationOpts) ->
    ...
    case jesse:validate_definition(swag_server_wallet_utils:to_list(DefinitionName), Schema, Value, Options) of
        {ok, _} ->
            ok;
        {error, [Error]} ->
...
maybe_use_custom_validator(Value, Attr, State) ->
    case swag_server_wallet_custom_validator:validate_schema(Attr, Value, State) of
        pass  ->
...
do_check_value(Value, {?FORMAT, Type}, State) when
    ...
    Type =:= <<"binary">>
->
    case swag_server_wallet_common_validator:validate({type, erlang:binary_to_atom(Type, utf8)}, Value) of
        error ->
...
do_check_value(Value, {?FORMAT, Format}, State) when
    ...
    Format =:= <<"email">>
->
    case swag_server_wallet_common_validator:validate({format, erlang:binary_to_atom(Format, utf8)}, Value) of
        error ->
            jesse_error:handle_data_invalid(wrong_format, Value, State);
        _ ->
            State
...
validate_child_schema(Value, SchemaName, State) ->
    ...
    BadRef = swag_server_wallet_utils:to_list(Ref),
...
map_error_reason({'data_invalid', _Schema, Error, Data, Path0}) ->
    ...
    swag_server_wallet_utils:join(".", [Description, Path]).
...
get_error_path(Path0) ->
    Mapper = fun
        (N, Acc) when is_integer(N) ->
            ["[", swag_server_wallet_utils:to_binary(N), "]" | Acc];
        (X, Acc) ->
            [$., X | Acc]
    end,
    ...
    Path3 = swag_server_wallet_utils:to_binary(Path2),
...
get_error_description({missing_dependency, Dependency0}, _Value) ->
    Dependency = swag_server_wallet_utils:to_binary(Dependency0),
    <<"Missing dependency: ", Dependency/binary>>;
get_error_description(missing_required_property, Value) ->
    PropertyName = swag_server_wallet_utils:to_binary(Value),

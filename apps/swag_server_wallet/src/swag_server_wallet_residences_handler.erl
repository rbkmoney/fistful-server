...
%% basic handler
-module(swag_server_wallet_residences_handler).
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'GetResidence' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
get_response_spec('GetResidence', 200) ->
    {'Residence', 'Residence'};

get_response_spec('GetResidence', 400) ->
    {'BadRequest', 'BadRequest'};

get_response_spec('GetResidence', 401) ->
    undefined;

get_response_spec('GetResidence', 404) ->
    undefined;

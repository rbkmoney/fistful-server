...
%% basic handler
-module(swag_server_wallet_currencies_handler).
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'GetCurrency' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
get_response_spec('GetCurrency', 200) ->
    {'Currency', 'Currency'};

get_response_spec('GetCurrency', 400) ->
    {'BadRequest', 'BadRequest'};

get_response_spec('GetCurrency', 401) ->
    undefined;

get_response_spec('GetCurrency', 404) ->
    undefined;

...
%% basic handler
-module(swag_server_wallet_wallets_handler).
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'GetWallet' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'ListWallets' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
get_request_spec('GetWalletAccount') ->
    [
        {'X-Request-ID', #{
            source => header,
            rules  => [{type, 'binary'}, {max_length, 32}, {min_length, 1}, true
...
get_response_spec('GetWallet', 400) ->
    {'BadRequest', 'BadRequest'};

get_response_spec('GetWallet', 401) ->
    undefined;

get_response_spec('GetWallet', 404) ->
    undefined;

get_response_spec('GetWalletAccount', 200) ->
    {'WalletAccount', 'WalletAccount'};

get_response_spec('GetWalletAccount', 400) ->
    {'BadRequest', 'BadRequest'};

get_response_spec('GetWalletAccount', 401) ->
    undefined;
...
get_response_spec('ListWallets', 401) ->
    undefined;
